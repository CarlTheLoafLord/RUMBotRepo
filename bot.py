...
###---------------------------------------------------------------------------- GAME STUFF
amazonlinks = ['https://www.amazon.com/Secret-Hitler/dp/B01JKD4HYC/',
               'https://www.amazon.com/CASOFU-Tortilla-Comfortable-Burrito-60inches/dp/B07QTHK8K9/',
               'https://www.amazon.com/How-Talk-Your-About-Safety/dp/045149492X/sr_1_1/',
               'https://www.amazon.com/Cooking-Semen-Delicious-Recipes-Inappropriate/dp/1091069387/',
               'https://www.amazon.com/Otamatone-Touch-Sensitive-Electronic-Musical-Instrument/dp/B00MRJ8GXK/',
               'https://www.amazon.com/Dodecagon-12-Side-Relieves-Depression-Children/dp/B071CFZ4BD/',
               'https://www.amazon.com/CelebriDucks-Harry-Ponder-Wizard-RUBBER/dp/B01GSOTFMA/',
               'https://www.amazon.com/PIZOFF-Pullover-Drawstring-Sweatshirts-AM006-01-L/dp/B07HGYVM55/',
               'https://www.amazon.com/Rubies-Conical-Alien-Skull-Flesh/dp/B001K3A45M/r',
               'https://www.amazon.com/Catholic-Hipster-Handbook-audiobook/dp/B07CVKHCLR/',
               'https://www.amazon.com/Simulation-Pillow-Lumbar-Cushion-Stuffed/dp/B07SHP29DM/',
               'https://www.amazon.com/Bluetooth-BCELIFE-Hands-Free-Rechargeable-Gifts%C2%A3%C2%A8Gray%C2%A3/dp/B075KTTKXS/',
               'https://www.amazon.com/Villager-News/dp/B01I2NXNE6/',
               'https://www.amazon.com/Mattel-Games-FPD61-Minecraft-Card/dp/B074RCV4HQ/',
               'https://www.amazon.com/Jay-Franco-Minecraft-Quilt-Piece/dp/B072L3GMZV/',
               'https://www.amazon.com/Basics-Of-Fly-Fishing/dp/B07HKSTWBX/',
               'https://www.amazon.com/Clip-Roblox-Saving-Memes-Game/dp/B0837JN2FC/']
...
async def checkSuggestions():
    ...
    while True:
        for message in bot.suggestQueue:
            ...
            approvals = approvalsObject.count - (bot.user in set(
                await approvalsObject.users().flatten()))  # gets # of yes reactions that isn't the bot
            denials = denialsObject.count - (bot.user in set(
                await denialsObject.users().flatten()))  # gets # of no reactions that isn't the bot
            timeLimit = datetime.timedelta(seconds=6 * 3600 * (1 - (
                        approvals + denials) / bot.memberCount))  # math to figure out the time limit of the suggestion - 0 people reacted yet=6 hrs
...
async def on_message(message):
    ...
    if message.content.startswith(prefix + 'embarrassme'):
    ...
    if command.startswith(prefix + 'info'):
        ...
        embedVar.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/738951182969602078/740711482391658567/botpic_2.png")
    ...
    if command.startswith(prefix + 'server'):
        print("Server Called")
        embedVar = discord.Embed(title="Republic of United Members",
                                 description="Casual server focused around fairness and democracy. ")
        embedVar.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/738951182969602078/740711351152017458/e20176f3cfe1fc2d0edc24005d749a8b_2.png")
        embedVar.add_field(name="Creation Date:",
                           value=message.guild.created_at.strftime("%b") + " " + message.guild.created_at.strftime(
                               "%d") + ", " + message.guild.created_at.strftime("%Y"), inline=True)
        embedVar.add_field(name="Server Age:", value=str(
            (message.channel.guild.created_at.utcnow() - message.channel.guild.created_at).days) + " Days", inline=True)
    ...
    if command.startswith(prefix + 'help'):
        print("Help Called")
        embedVar = discord.Embed(title="RUM Bot Command List", description="List containing all bot commands.",
                                 color=0xfb00ff)
        embedVar.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/738951182969602078/740711482391658567/botpic_2.png")
        num_lines = sum(1 for line in open('help.txt'))
        for helpNum in range((num_lines // 2)):
            embedVar.add_field(name=await getLine('help.txt', 2 * helpNum + 1),
                               value=await getLine('help.txt', 2 * helpNum + 2), inline=True)
        embedVar.set_footer(
            text="Any questions? Ask one of the contributors! Any Suggestions? Put them in #suggestions!")
        await message.channel.send(embed=embedVar)
    ...
    if command.startswith(prefix + 'rule '):
        ...
        if 1 <= ruleNum <= 9:
            embedVar = discord.Embed(title=await getLine("rules.txt", 2 * ruleNum - 1),
                                     description=await getLine("rules.txt", 2 * ruleNum), color=0xEC00FF)
            await message.channel.send(embed=embedVar)
        else:
    ...
    if command.startswith(prefix + "warn") or command.startswith(prefix + "strike"):
        ...
        if message.guild.get_role(
                736316470098657342) in message.author.roles or message.author.id == 369988289354006528 or message.author.id == 317456004843438082:
    ...
    if command.startswith(prefix + "rockpaperscissors") or command.startswith(prefix + "rps"):
        ...
        try:
            ...
            def challengerCheck(reaction, user):
                return reaction.message.id == challengerMsg.id and user == challenger and (
            ...
            def opponentCheck(reaction, user):
                return reaction.message.id == opponentMsg.id and user == opponent and (
                            str(reaction.emoji) == '✊' or str(reaction.emoji) == '🖐️' or str(reaction.emoji) == '✌️')
...
async def on_member_join(member):
    ...
    if member.bot == False:
        await member.guild.gzet_channel(739647916905332846).send(
